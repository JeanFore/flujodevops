import { Application, Candidate, Interview, InterviewStep, PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

type ApplicationWithCandidateDetails = Application & {
    candidate: Candidate;
...
export const getCandidatesByPositionService = async (positionId: number) => {
    try {
        ...
        return applications.map((app: ApplicationWithCandidateDetails) => ({
...
export const getInterviewFlowByPositionService = async (positionId: number) => {
    ...
    return {
        ...
        interviewFlow: {
            ...
            interviewSteps: positionWithInterviewFlow.interviewFlow.interviewSteps.map((step: InterviewStep) => ({
                id: step.id,
